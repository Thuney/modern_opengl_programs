...
vec3 CalculatePointLight(PointLight light, vec3 normal, vec3 view_direction)
{
	...
	vec3 diffuse_light = (light.diffuse_intensity * light.color) * material.diffuse_color * diffuse_projection; //Change swizzling at a later date
...
vec3 CalculateDirectionalLight(DirectionalLight light, vec3 normal, vec3 view_direction)
{
    ...
    vec3 ambient_light  = (light.ambient_intensity * light.color)  * material.ambient_color;
    vec3 diffuse_light  = (light.diffuse_intensity * light.color)  * diffuse_projection * material.diffuse_color;
